...
The generic, non-domain plumbing lives in **telegram-broadcast-kit** (a separate package, pinned by git tag in `package.json`: `github:edriso/telegram-broadcast-kit#v0.2.0`). NumNinjas consumes it for: the console logger, the root `.env` loader (`loadEnv`), the timezone day math (`dayOfYearIn`), the channel poster (`post`, with opt-in `parseMode: 'HTML'` for the context message), the quiz poll (`sendPoll` in `type: 'quiz'` mode, with `correctOptionId` + `explanation` and the poll-close clamping), the bidi isolate helper (`ltrIsolate`, UAX #9) that pins the poll's plain-text question and options left-to-right, the node-cron registry + error containment (`Scheduler`, `runJob`), and the `/health` server. The kit gained quiz-poll and opt-in `parseMode` support in v0.2.0, which is exactly what this bot needs.
...
- **Poll text is pinned left-to-right.** A poll's question and options are plain text with no `parse_mode`, so Telegram lays them out using the **reader's** app-language direction, not the text's. For a follower whose Telegram is set to an RTL language (Arabic, Hebrew, ...), our English polls would mirror: "Which answer is correct? 🥷" renders "🥷 ?Which answer is correct", and a number-first option like "1 sweet" flips to "sweet 1". `pollQuestion`/`pollOptions` in `src/lib/format.ts` wrap each string in the kit's `ltrIsolate` (a Unicode LTR isolate, UAX #9) to force left-to-right for every reader. A leading LRM does not help here, because the base direction is forced by locale rather than inferred from the first strong character; only an explicit isolate overrides it. The HTML context message is unaffected (Telegram messages infer direction from their content), so only the poll strings need this.
...
- The poll helpers: `pollQuestion` stays under 300 chars and is pinned left-to-right (wrapped in a Unicode LTR isolate, U+2066 ... U+2069); `pollOptions` returns the four real answers in order, each pinned left-to-right and still within Telegram's 100-char poll-option limit.
