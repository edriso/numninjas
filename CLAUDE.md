...
- `mulberry32`/`seedFrom` (`rng.ts`): deterministic per seed, in-range `int` (including a single-value range and the `max < min` throw), `pick` throwing on an empty array, non-mutating `shuffle` (including empty and single-element arrays), and order- and adjacency-sensitive seeds.
- `questionProblems`/`isValidQuestion` (`validate.ts`): accepts a well-formed question and flags each violation directly (bad id prefix, wrong option count, duplicate or empty or over-long option, out-of-range `correctIndex`, over-long explanation or prompt, em/en-dashes, empty prose, and a blown render budget).
- `unit`/`wrongInts` (`helpers.ts`): correct singular/plural (including 0 and an irregular plural); and three distinct, non-negative distractors that prefer the misconception candidates, skip negatives/duplicates/the answer itself, and fall back to stepping out from the answer.
